...
<header class="fixed w-full top-0 z-50 transition-all duration-300" 
    ...
    <div class="max-w-7xl mx-auto px-6 py-5 flex justify-between items-center">
        ...
        <a href="{{ route('home') }}" class="inline-block">
            <img x-show="!scrolled" src="{{ asset('images/logo/' . (isset($navDark) && $navDark ? 'logo-naranja.webp' : 'logo-gris.webp')) }}" 
        ...
        <nav class="hidden md:flex items-center space-x-8">
            <a href="{{ route('home') }}" 
               ...
               :class="scrolled ? 'text-boom-gray' : '{{ isset($navDark) && $navDark ? 'text-white' : 'text-boom-gray' }}'">Inicio</a>
            <a href="{{ route('services.index') }}" 
               class="text-sm font-medium uppercase tracking-wide hover:text-boom-orange transition-colors"
               :class="scrolled ? 'text-boom-gray' : '{{ isset($navDark) && $navDark ? 'text-white' : 'text-boom-gray' }}'">Servicios</a>
            <a href="{{ route('portfolio.index') }}" 
               class="text-sm font-medium uppercase tracking-wide hover:text-boom-orange transition-colors"
               :class="scrolled ? 'text-boom-gray' : '{{ isset($navDark) && $navDark ? 'text-white' : 'text-boom-gray' }}'">Portfolio</a>
            
            {{-- Páginas dinámicas --}}
            @foreach($menuPages as $menuPage)
            <a href="{{ route('page.show', $menuPage->slug) }}" 
               class="text-sm font-medium uppercase tracking-wide hover:text-boom-orange transition-colors"
               :class="scrolled ? 'text-boom-gray' : '{{ isset($navDark) && $navDark ? 'text-white' : 'text-boom-gray' }}'">{{ $menuPage->title }}</a>
            @endforeach
            
            <a href="{{ route('blog.index') }}" 
                class="text-sm font-medium uppercase tracking-wide hover:text-boom-orange transition-colors"
                :class="scrolled ? 'text-boom-gray' : '{{ isset($navDark) && $navDark ? 'text-white' : 'text-boom-gray' }}'">Blog</a>
            <a href="{{ route('contact') }}" 
               class="text-sm font-medium uppercase tracking-wide hover:text-boom-orange transition-colors"
               :class="scrolled ? 'text-boom-gray' : '{{ isset($navDark) && $navDark ? 'text-white' : 'text-boom-gray' }}'">Contacto</a>
        ...
        <!-- Mobile Menu Button -->
        <button @click="menuOpen = !menuOpen" class="md:hidden" :class="scrolled ? 'text-boom-gray' : '{{ isset($navDark) && $navDark ? 'text-white' : 'text-boom-gray' }}'">
